...
local function D(s)
    if base.type(_DEBUG) == "function" then _DEBUG(s) elseif _DEBUG then print(s) end
end

function dump(t, seen)
    if seen == nil then seen = {} end
    local typ = base.type(t)
    local st = "(" .. typ .. ") " .. tostring(t)
...
local function mapLocaleMonth( m )
    ...
    if monthNameMap[ml] ~= nil then -- cached result?
        D("mapLocaleMonth(" .. ml .. ") cached result=" .. monthNameMap[ml])
...
local function xp_parse_time( t )
    ...
    if p[3] ~= nil then 
        ...
        D(string.format("Found p1=%s, p2=%s, sep=%s, rem=%s", p[1], p[2], p[3], p[4]))
        local sep = p[2]
        t = p[4] or ""
        D(string.format("Scanning for 3rd part from: '%s'", t))
        ...
        p[5]=t p[6]=p[6]or"" D(string.format("p=%s,%s,%s,%s,%s", unpack(p)))
        ...
        D(string.format("Parsed date year=%s, month=%s, day=%s", tostring(tt.year), tostring(tt.month), tostring(tt.day)))
    else
        -- YYYYMMDD?
        ...
        if p[3] ~= nil then
            ...
            if p[2] ~= nil then
                D(string.format("Matches %%c format, 1=%s,2=%s,3=%s", p[1], p[2], p[3]))
                tt.day = p[2]
    ...
    D(string.format("Scanning for time from: '%s'", t))
    local hasTZ = false
    local sep = nil
    ...
    if p[1] ~= nil then
        ...
        D(string.format("Parsed time is %s:%s:%s", tt.hour, tt['min'], tt.sec))
    ...
    local delta = 0
    D("Checking for offset from '" .. t .. "'")
    p = { t:match("%s*([+-])(%d+)(.*)") }
    if p[2] ~= nil then
        D("Parsing offset from " .. t .. ", first part is " .. p[2])
        local sign = p[1]
        delta = tonumber(p[2]) 
...
local function xp_date_add( a ) 
    ...
    if a[6] ~= nil or a[7] ~= nil then
        D("Applying delta months and years to " .. tm)
        local d = os.date("*t", tm)
        d.month = d.month + ( tonumber( a[6] ) or 0 )
        d.year = d.year + ( tonumber( a[7] ) or 0 )
        D(string.format("Normalizing month,year=%d,%d", d.month, d.year))
        while d.month < 1 do
            d.month = d.month + 12
            d.year = d.year - 1
        end
...
local function xp_rtrim(s)
    if base.type(s) ~= "string" then error("String required") end
    return s:gsub("%s+$", "")
end
...
local function skip_white( expr, index )
    D("skip_white from " .. index .. " in " .. expr)
...
local function scan_numeric( expr, index )
    D("scan_numeric from " .. index .. " in " .. expr)
    local len = string.len(expr)
    local ch, i
    local val = 0
    local base = 0
...
local function scan_fref( expr, index, name )
    D("scan_fref from " .. index .. " in " .. expr)
    local len = string.len(expr)
    local args = {}
    local parenLevel = 1
    local ch
    ...
    while ( true ) do
        ...
        ch = string.sub(expr, index, index)
        if (ch == ')') then
            D("scan_fref: Found a closing paren while at level " .. parenLevel)
            parenLevel = parenLevel - 1
            if (parenLevel == 0) then
                subexp = xp_trim( subexp )
                D("scan_fref: handling end of argument list with subexp=" .. subexp)
                ...
                index = index + 1
                D("scan_fref returning, function is " .. name .. " with " .. table.getn(args) .. " arguments: " .. dump(args))
...
local function scan_aref( expr, index, name )
    D("scan_aref from " .. index .. " in " .. expr)
    local len = string.len(expr)
    local args = {}
    local parenLevel = 1
    local ch
    ...
    while ( true ) do
        ...
        ch = string.sub(expr, index, index)
        if (ch == ']') then
            D("scan_aref: Found a closing bracket, subexp=" .. subexp)
            args = _comp(subexp)
            D("scan_aref returning, array is " .. name)
...
local function scan_vref( expr, index )
    D("scan_vref from " .. index .. " in " .. expr)
    local len = string.len(expr);
    local ch, k
    local name = ""
    while (index <= len) do
...
local function scan_expr( expr, index )
    D("scan_expr from " .. index .. " in " .. expr)
    local len = string.len(expr)
    local ch, k
    local st = ""
    local parenLevel = 0
    ...
    while (index <= len) do
        ...
        if (ch == ')') then
            if (parenLevel == 0) then
                D("scan_expr parsing subexpression=" .. st)
                local r = _comp( st )
...
local function scan_unop( expr, index )
    D("scan_unop from " .. index .. " in " .. expr)
    local len = string.len(expr)
    local ch, k
    ch = string.sub(expr, index, index)
...
local function scan_binop( expr, index )
    D("scan_binop from " .. index .. " in " .. expr)
    local len = string.len(expr)
    local matched = false
...
local function parse_rpn( lexpr, expr, index, lprec )
    D("parse_rpn: parsing " .. expr .. " from " .. index .. " prec " .. lprec .. " lhs " .. dump(lexpr))
    local len = string.len(expr)
    local stack = {}
...
local function resolve( name, context )
    ...
    D("resolve: resolved " .. tostring(name))
    local k
    local i = 1
    local m = context
    repeat
...
local function check_operand_type(v, allowed, err)
    ...
    if base.type(allowed) == "string" then
        if vt == allowed then return true else evalerror(err) end
    end
...
local function coerce(val, typ)
    ...
    D("coerce: attempt (" .. vt .. ")" .. tostring(val) .. " to (" .. typ .. ")")
    ...
    elseif typ == "number" then
        ...
        elseif vt == "string" then
            ...
            if n ~= nil then return n else evalerror("Coersion of " .. val .. "from string to number failed") end
        end
    end
...
local function fetch( stack, ctx ) 
    ...
    if e == nil then evalerror("Missing required operand") end
    D("fetch() got " .. base.type(e) .. " " .. dump(e))
    if base.type(e) == "table" then 
        if e.type == VREF then
            ...
            if (e.index ~= nil) then
                ...
                local ix = _run(e.index, ctx, stack)
                D("fetch: vref " .. e.name .. " applying subscript " .. tostring(ix))
                if ix ~= nil then
                    v = v[ix]
...
_run = function( ce, ctx, stack )
    ...
    while (index <= len) do
        local e = ce[index]
        D("_run: next element is " .. dump(e))
        if ( base.type(e) == "number" or base.type(e) == "string" ) then
            D("_run: " .. base.type(e) .. " value: " .. tostring(e))
            v = e
        elseif (base.type(e) == "table" and e.type == nil) then
            D("_run: subexpression: " .. dump(e))
            v = _run( e, ctx, stack )
            if (v == nil) then return nil end
        elseif (e.type == BINOP) then
            D("_run: handling BINOP " .. e.op)
            local v2
            if e.op == '.' then
                v2 = table.remove( stack, 1 )
                D("_run: subref lookahead is " .. dump(v2))
            else
            ...
            if e.op == '=' then
                ...
                D("_run: assignment lookahead is " .. dump(v1))
                if base.type(v1) ~= "table" or v1.type ~= VREF then evalerror("Invalid assignment", e.pos) end
            else
                v1 = fetch(stack, ctx)
            end
            D("_run: operands are " .. dump(v1) .. ", " .. dump(v2))
            if (e.op == '.') then
                D("_run: descend to " .. dump(v2))
                check_operand_type(v1, "table", "Cannot subreference a " .. base.type(v1))
                if base.type(v2) ~= "table" or v2.type ~= VREF then evalerror("Invalid subreference", e.pos) end
            ...
            elseif (e.op == '%') then
                ...
                v = v1 % v2
            elseif (e.op == '&') then
                if base.type(v1) == "number" and base.type(v2) == "number" then
            ...
            elseif e.op == '=' then
                D("_run: making assignment to " .. v1.name)
        ...
        elseif (e.type == UNOP) then
            -- Get the operand
            D("_run: handling unop, stack has " .. table.getn(stack))
            v = fetch(stack, ctx)
        ...
        elseif (e.type == FREF) then
            -- Function reference
            D("_run: Handling function " .. e.name .. " with " .. table.getn(e.args) .. " arguments passed");
            ...
            D("_run: finished " .. e.name .. "() call, status=" .. tostring(status) .. ", result=" .. dump(v))
            if not status then 
                if base.type(v) == "table" and v.type == "evaluation" then
                    v.location = e.pos
                    error(v) -- that one of our errors, just pass along
                end
                error("Execution of function " .. e.name .. "() returned an error: " .. tostring(v))
            end
        elseif (e.type == VREF) then
            D("_run: handling vref, name=" .. e.name)
        ...
        D("_run: pushing result to stack: " .. dump(v))
    ...
    D("_run: finished, stack has " .. table.getn(stack) .. ": " .. dump(stack))
    if (table.getn(stack) > 0) then
        return fetch(stack, ctx) -- return first element. Maybe return multiple???
    end
    return nil
end
